...
class ValuationAggregate(str, PyEnum):
    ARR = "arr"
    REVENUE = "revenue"
    EBITDA = "ebitda"
...
class TargetAnchor(Base):
    ...
    m_market_entry: Mapped[float] = mapped_column(Float, nullable=False)
...
class RunComp(Base):
    ...
    comp_snapshot_id: Mapped[int] = mapped_column(ForeignKey("comp_snapshots.id"), nullable=False)
